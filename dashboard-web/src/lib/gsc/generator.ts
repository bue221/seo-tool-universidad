...
export async function getGscDataset(
	...
	const prevSeries = generateSeries(
		property,
		range,
		mulberry32(baseSeed ^ 0xa1 ^ 0xf6),
	);
	const previousPeriodTotals = scaleDown(
		sumSeries(prevSeries),
		0.85 + 0.3 * (seriesRand() - 0.5),
	);
...
/**
 * Normalize any URL-like input to a host/domain token.
 *
 * - lowercases
 * - strips protocol, path, query, and fragment
 * - keeps subdomains intact (`blog.example.com` stays distinct)
 */
export function normalizeDomain(input: string): string {
	...
	try {
		const candidate = /^[a-z][a-z0-9+.-]*:\/\//.test(decodedInput)
			? decodedInput
			: `https://${decodedInput}`;
		return new URL(candidate).hostname;
	} catch {
		// Fallback for malformed values stored in snapshots.
		return decodedInput
			.replace(/^https?:\/\//, "")
			.split(/[/?#]/)[0]!
			.replace(/\/+$/, "");
...
function allocateMetrics(
	...
	return keys.map((key, i) => {
		...
		const ctrFactor = isBrand
			? randRange(rand, 1.5, 2.5)
			: randRange(rand, 0.5, 1.0);
		const posOffset = isBrand ? randRange(rand, -4, 0) : randRange(rand, 0, 8);

		const impressions = Math.max(
			1,
			Math.round(totals.impressions * weights[i]!),
		);
