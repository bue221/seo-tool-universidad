...
import { SiteTreeSection } from "../_components/SiteTreeSection";
...
export default async function SnapshotDetailPage({ params }: Props) {
	...
	return (
		<div className="space-y-8">
			...
			<SectionCard eyebrow={tDetail("detailsEyebrow")} bodyClassName="p-0">
				<Tabs defaultValue="pagespeed" className="w-full">
					<TabsList className="w-full justify-start overflow-x-auto rounded-none border-b border-border bg-transparent px-4">
						<TabsTrigger value="pagespeed">
							{tDetail("tabs.pagespeed")}
						</TabsTrigger>
						<TabsTrigger value="onpage">{tDetail("tabs.onpage")}</TabsTrigger>
						{snapshot.result.scraper?.woorank ? (
							<TabsTrigger value="woorank">
								{tDetail("tabs.woorank")}
							</TabsTrigger>
						) : null}
						<TabsTrigger value="tracking">
							{tDetail("tabs.tracking")}
						</TabsTrigger>
						<TabsTrigger value="keywords">
							{tDetail("tabs.keywords")}
						</TabsTrigger>
						<TabsTrigger value="sentiment">
							{tDetail("tabs.sentiment")}
						</TabsTrigger>
						<TabsTrigger value="recommendations">
							{tDetail("tabs.recommendations")}
						</TabsTrigger>
						<TabsTrigger value="structure">
							{tDetail("tabs.structure")}
						</TabsTrigger>
						<TabsTrigger value="observability">
							{tDetail("tabs.observability")}
						</TabsTrigger>
					</TabsList>
					<div className="p-6">
						...
						<TabsContent value="recommendations">
							<RecommendationsSection result={snapshot.result} />
						</TabsContent>
						<TabsContent value="structure">
							<SiteTreeSection result={snapshot.result} />
						</TabsContent>
						<TabsContent value="observability">
							<ObservabilitySection result={snapshot.result} />
						</TabsContent>
					</div>
				</Tabs>
			</SectionCard>
