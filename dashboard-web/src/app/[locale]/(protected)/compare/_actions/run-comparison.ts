...
> {
  ...
  const settled = await Promise.allSettled(
    dedup.map((d) => withTimeout(runFullAudit(d.original), PER_AUDIT_TIMEOUT_MS)),
  );
  ...
  const entries: ComparisonEntry[] = settled.map((res, i) => {
    ...
    if (res.value === null) {
      return { url: meta.url, isYou: meta.isYou, status: 'error', error: 'UPSTREAM_BOTH_FAILED' };
    }
  ...
  return {
    ok: true,
    data: { ranAt: new Date().toISOString(), entries },
  };
}
