...
async function PropertyGrid({ properties }: { properties: string[] }) {
  ...
  const datasets = await Promise.all(properties.map((p) => getGscDataset(p, 28)));
  return (
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {datasets.map((d) => (
        <PropertyCard key={d.property} property={d.property} totals={d.totals} rangeLabel={rangeLabel} />
      ))}
    </div>
  );
}
